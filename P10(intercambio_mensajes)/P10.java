	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				System.out.println("Server Received: " + message);//acepta la petición de comunicación
	...
	public void run() {
		try {
			for (int x = 0; x < 5; x++) {
				...
				oos = new ObjectOutputStream(socket.getOutputStream());
				oos.writeObject("Client Message " + x);//manda la petición de comunicación
				ois = new ObjectInputStream(socket.getInputStream());//recibe el fragmento
